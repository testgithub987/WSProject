...
@when(u'Set HEADER param response mlrs-token as "{mlrs_token_type}"')
def step_impl(context, mlrs_token_type):
    http_request_header['mlrs-token'] = mlrs_token_type
    
@when(u'Set HEADER param response mlrs-email as "{mlrs_email_type}"')
def step_impl(context, mlrs_email_type):
    http_request_header['mlrs-email'] = mlrs_email_type
...
def step_impl(context, body_parsing_for):
    current_json = global_general_variables['response_full'].json()
    print current_json
    if 'Test' == body_parsing_for:
        if json.dumps(current_json).find("Test") == True:
            print "success"
